...
impl OperationsTrait for AverageSequence {
    ...
    fn execute_impl(&self, _image: &mut Image) -> Result<(), ImageErrors> {
        Err(ImageErrors::GenericStr("Average Sequence requires the full stack; call via execute_multiple"))
    }
    ...
    fn execute_multiple(&self, images: &mut Vec<Image>) -> Result<(), ImageErrors> {
        if images.len() < 2 {
            return Err(ImageErrors::GenericStr("Average Sequence requires at least 2 images."));
        }
        ...
        for img in &other_images {
            if img.dimensions() != b_dims || img.depth() != b_depth || img.colorspace() != b_colorspace {
        ...
        for frame_idx in 0..base_image.frames_ref().len() {
            let num_channels = base_image.frames_ref()[frame_idx].channels_ref(b_colorspace, false).len();
            ...
            for c_idx in 0..num_channels {
                ...
                match bit_type {
                    BitType::U8 => {
                        ...
                        for img in &other_images {
                            let chan = &img.frames_ref()[frame_idx].channels_ref(b_colorspace, false)[c_idx];
                            other_slices.push(chan.reinterpret_as::<u8>()?);
                        }
                        let base_chan = &mut base_image.frames_mut()[frame_idx].channels_mut(b_colorspace, false)[c_idx];
                        average_channels::<u8>(base_chan.reinterpret_as_mut::<u8>()?, &other_slices);
                    }
                    BitType::U16 => {
                        let mut other_slices = Vec::with_capacity(other_images.len());
                        for img in &other_images {
                        ...
                        let base_chan = &mut base_image.frames_mut()[frame_idx].channels_mut(b_colorspace, false)[c_idx];
                        average_channels::<u16>(base_chan.reinterpret_as_mut::<u16>()?, &other_slices);
                    }
                    BitType::F32 => {
                        let mut other_slices = Vec::with_capacity(other_images.len());
                        for img in &other_images {
                        ...
                        let base_chan = &mut base_image.frames_mut()[frame_idx].channels_mut(b_colorspace, false)[c_idx];
                        average_channels::<f32>(base_chan.reinterpret_as_mut::<f32>()?, &other_slices);
                    }
