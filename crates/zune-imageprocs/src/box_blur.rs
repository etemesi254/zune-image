...
pub(crate) fn box_blur_inner_nx<T, const N: usize>(
    in_rows: &[&mut [T]; N],
    out_rows: &mut [&mut [T]; N],
    width: usize,
    radius: usize,
) where
    T: Copy + NumOps<T>,
    u32: std::convert::From<T>,
{
    ...
    // OPTIMIZER HINT: Tell LLVM that all slices are at least `width` long.
    // This allows the compiler to completely elide bounds checks in the loops below.
    for i in 0..N {
    ...
    for i in 0..N {
        acc[i] += (radius as u32) * u32::from(in_rows[i][0]);
    }

    for x in 0..=safe_r {
        for i in 0..N {
            acc[i] += u32::from(in_rows[i][x]);
        }
    }
    ...
    if radius > safe_r {
        ...
        for i in 0..N {
            acc[i] += diff * u32::from(in_rows[i][width - 1]);
        }
    }

    for i in 0..N {
        out_rows[i][0] = T::from_u32(fastdiv_u32(acc[i], m_radius));
    }
    ...
    if width > diameter {
        ...
        let mut l = [0u32; N];
        for i in 0..N {
            l[i] = u32::from(in_rows[i][0]);
        }

        for x in 1..=radius {
            let right_idx = x + radius;
            for i in 0..N {
                acc[i] = acc[i] + u32::from(in_rows[i][right_idx]) - l[i];
                out_rows[i][x] = T::from_u32(fastdiv_u32(acc[i], m_radius));
            }
        }

        // 2. MIDDLE
        // Because of the `assert!` at the top, we don't need manual sub-slicing.
        let mid_len = width - diameter;
        for x in 0..mid_len {
            for i in 0..N {
                acc[i] = acc[i] + u32::from(in_rows[i][x + diameter]) - u32::from(in_rows[i][x]);
                out_rows[i][x + radius + 1] = T::from_u32(fastdiv_u32(acc[i], m_radius));
            }
        }
        ...
        let mut e = [0u32; N];
        for i in 0..N {
            e[i] = u32::from(in_rows[i][width - 1]);
        }

        for x in (width - radius)..width {
            let left_idx = x - radius - 1;
            for i in 0..N {
                acc[i] = acc[i] + e[i] - u32::from(in_rows[i][left_idx]);
                out_rows[i][x] = T::from_u32(fastdiv_u32(acc[i], m_radius));
            }
        }
    } else {
        ...
        for x in 1..width {
            ...
            for i in 0..N {
                acc[i] =
                    acc[i] + u32::from(in_rows[i][right_idx]) - u32::from(in_rows[i][left_idx]);
                out_rows[i][x] = T::from_u32(fastdiv_u32(acc[i], m_radius));
            }
        }
    }
