...
pub extern "C" fn zil_imread(
    ...
    if file.is_null() {
        unsafe {
            *status = ZStatus::new("In file is null".to_string(), ZStatusType::ZilMallocFailed)
        }
        return ERROR;
    }
...
pub extern "C" fn zil_read_headers_from_file(
    file: *const c_char, metadata_storage: *mut ZImageMetadata, status: *mut ZStatus,
) -> i64 {
    if status.is_null() {
        return STATUS_IS_NULL;
    }
    ...
    match std::fs::read(file_cstr) {
        Ok(bytes) => {
            zil_read_headers_from_memory(bytes.as_ptr(), bytes.len() as _, metadata_storage, status)
        }
        Err(error) => {
            unsafe {
...
pub extern "C" fn zil_read_headers_from_memory(
    input: *const u8, input_size: usize, image_metadata: *mut ZImageMetadata, status: *mut ZStatus,
) -> i64 {
    if status.is_null() {
        return STATUS_IS_NULL;
    }
