...
assembled from committed coefficient data. These preview pixels may change after
later scans, so they are exposed through `decoded_preview_output_bytes()`,
...
- `decoded_preview_output_bytes()` / `decoded_preview_scanlines()` describe a
  ...
  browser-style display, but those pixels may be replaced by later scans. These
  methods return `None` for non-progressive images and `Some(0)` for progressive
  images before the first preview is rendered.
  ...
  match decoder.decode_into(&mut pixels) {
    ...
    Err(error) if error.is_recoverable_eof() => {
      ...
      if stable_bytes > 0 {
        ...
        // `preview_scanlines` is full height once a preview is available, and
        // `preview_scans` tells how many completed scans it represents.
