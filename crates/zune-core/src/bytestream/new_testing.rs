...
impl<T: ZByteIoTrait> ZReader<T> {
    ...
    #[inline]
    pub fn get_fixed_bytes_or_err<const N: usize>(&mut self) -> Result<[u8; N], ZByteIoError> {
    ...
    #[inline]
    pub fn peek_exact_bytes_or_err<const N: usize>(&mut self) -> Result<[u8; N], ZByteIoError> {
        let mut byte_store: [u8; N] = [0; N];
        match self.inner.peek_exact_bytes(&mut byte_store) {
            Ok(_) => Ok(byte_store),
            Err(e) => Err(e)
        }
    }
