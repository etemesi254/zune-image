...
#[test]
fn test_similarity_gz()
{
    let path = env!("CARGO_MANIFEST_DIR").to_string() + "/tests/gzip";
    ...
    for file in dirs.flatten()
    {
        ...
        {
            let data = read(&file.path()).unwrap();
            let flate_data = decode_writer_flate_gz(&data);
            let zune_data = decode_writer_zune_gz(&data);
